...
# The corporate template shipped with the repo (used unless a caller overrides it).
DEFAULT_TEMPLATE = Path(__file__).resolve().parent.parent / "assets" / "clear-sky-template.docx"
...
def add_step_title(doc, text: str) -> None:
    """Body Text paragraph; the leading Gherkin keyword is uppercase, bold, teal.
...
def build_doc(manifest: dict, output: Path, template: Path | None = DEFAULT_TEMPLATE) -> None:
